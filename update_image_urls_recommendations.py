"""
Script to update url_gambar in banten_beach_recommendations.json as well.
Uses the same slug -> image mapping.
"""
...
def main():
    path = "output/banten_beach_recommendations.json"
    
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    
    print(f"Total entries in recommendations JSON: {len(data)}")
    ...
    for entry in data:
        slug = entry.get("slug", "")
        beach_name = entry.get("pantai", "unknown")
        
        if slug in SLUG_TO_IMAGE:
            old_url = entry.get("url_gambar", "")
            new_url = BASE + SLUG_TO_IMAGE[slug]
            entry["url_gambar"] = new_url
            status = "UPDATED" if old_url != new_url else "SAME"
            print(f"  {status}: {beach_name} ({slug})")
            updated += 1
        else:
